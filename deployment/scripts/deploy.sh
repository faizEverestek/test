...
main() {
  ...
  # Step 1: Deploy Infra
  bash deployment/scripts/infra.sh "$ENV" 2>&1 | tee -a  "$LOG_FILE"

  # Step 2: Deploy Green Environment
  bash deployment/scripts/green.sh "$ENV" 2>&1 | tee -a  "$LOG_FILE"

  # Step 3: Run Sanity Check
  bash deployment/scripts/sanity.sh "$ENV" 2>&1 | tee -a  "$LOG_FILE"
  ...
  if [[ ! -f "$SANITY_RESULT_FILE" ]]; then
    ...
    if [[ "$STATUS" == "SUCCESS" ]]; then
      ...
      bash deployment/scripts/blue.sh "$ENV" 2>&1 | tee -a  "$LOG_FILE"
    else
